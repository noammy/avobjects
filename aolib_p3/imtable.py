...
PUBLIC_URL = 'http://people.eecs.berkeley.edu/~owens/tab'
#WWW_PATH = '/home/eecs/owens/public_html/tab'
...
def get_url():
    name = ut.hostname()
    if name == 'zzz':
        return 'http://128.32.43.210/tab'
    elif name in ['knish', 'knodel', 'knack', 'kale', 'kitkat']:
        return 'http://people.eecs.berkeley.edu/~owens/tab'


def get_www_path():
    name = ut.hostname()
    if name == 'zzz':
        return '/home/ao/www/tab'
    elif name in ['knish', 'knodel', 'knack', 'kale', 'kitkat']:
        return '/home/eecs/owens/public_html/tab'
    else:
        return 'asdf'
...
def show_table(table, title=None, rows_per_page=100, show=False,
    ...
    use_www = use_www and os.path.exists(get_www_path())
...
def transpose_table(lsts, pad=''):
    return list(map(list, ut.zip_pad(pad, *lsts)))
...
def cycle_html(im_fnames_table, group_name, start):
    im_list_js = js_list([js_list(list(map(ut.quote, row)))
                          for row in im_fnames_table])
    id = str(random.random())
    group_name = str(random.random()) if group_name is None else group_name
    default = im_fnames_table[start[0]][start[1]] if len(
        im_fnames_table) and len(im_fnames_table[0]) else ''
...
def paginate_table(table_rows, rows_per_page, title, output_dir):
    ...
    page_names = ['index.html'] + ['page_%d.html' %
                                   i for i in range(2, 1+len(split_table))]
    ...
    for i in range(len(split_table)):
        ...
        if len(split_table) == 1:
            ...
            footer = ''
            footer += ("Back " if i ==
                       0 else "<a href = '%s'>Back</a> " % page_names[i-1])
            footer += ("Next " if i == -1 + len(page_names)
            ...
            footer += '<br><br><br><br>'
        ut.make_file(page_paths[i], "<html><head>%s<title>%s</title></head><body>%s<br>%s</html>" %
...
def html_from_cell(x, output_dir):
    if type(x) == type(''):
        ...
        if x[0] in ('img', 'img_mv', 'img-png'):
            ...
            seq_fname = make_temp_animated_gif(
                x[1], duration=duration, dir=output_dir)
...
def save_helper(xxx_todo_changeme):
    (fname, x) = xxx_todo_changeme
    ig.save(fname, x)
...
def make_temp_animated_gif_cmdline(seq, duration=0.5, dir=None, tmp_ext='.ppm', seq_fname=None):
    ...
    ut.parmap(save_helper, list(zip(fnames, seq)))
...
def make_video_helper(xxx_todo_changeme1):
    (i, x, in_dir, tmp_ext) = xxx_todo_changeme1
...
def make_video(out_fname, ims, fps, tmp_ext='.ppm', sound=None):
    ...
    with ut.temp_file('.wav', delete_on_exit=True) as tmp_wav:
        if sound is None:
            sound_flags = ''
        else:
            if type(sound) != type(''):
                sound.save(tmp_wav)
                sound = tmp_wav
            sound_flags = '-i "%s" -shortest -acodec aac' % sound
...
def sync_with_fps(ims, timestamps, target_fps, max_lag_sec=0.06):
    ...
    while i < len(ims):
        vid.append(ims[i])
        def video_time(): return (len(vid) - 1) * (1./target_fps)
...
class Video:
    def __init__(self, ims=None, fps=1, sound=None, timestamps=None,
                 ...
                 loop=False, speed=False, play_button=True):
        self.__video__ = True
    ...
    def make_html(self, out_dir, ext='.mp4'):
        ...
        loop_flag = '' if not self.loop else 'loop="loop"'

        if self.speed:
            speed_html = """<td>Speed: <input type = "text" size = "5" value = "1" oninput = "document.getElementById('%(vid_id)s').playbackRate = parseFloat(event.target.value);"></td>""" % locals(
            )
        else:
            speed_html = ''

        if not self.play_button:
            # bug: applies to *all* videos
            play_style = "<style>video::-webkit-media-controls-overlay-play-button { display: none; }</style>"
            onclick_flag = """ onclick = "document.getElementById('%(vid_id)s').play()" """ % locals(
            )
            controls_flag = ''
        else:
            play_style = ''
    ...
    <table>
      <tr>
        <td>
          <video %(controls_flag)s %(loop_flag)s id = "%(vid_id)s" onplay = "vid_focus = null;" onpause = "vid_focus = document.getElementById('%(vid_id)s');" onloadedmetadata = "var t = getParameterByName('videoStart'); if (t != null) { this.currentTime = t; } if (getParameterByName('videoAutoplay'))" { this.play(); } %(onclick_flag)s>
